...
async function checkPricingFields() {
  try {
    console.log('Checking if pricing fields exist in client_property_assignments...');
    ...
    if (error && error.message.includes('column')) {
      ...
      console.log(`
-- Add flexible pricing fields to client_property_assignments table
...
COMMENT ON COLUMN client_property_assignments.show_monthly_rent IS 'Whether to display monthly rent on client page';
COMMENT ON COLUMN client_property_assignments.custom_monthly_rent IS 'Custom monthly rent amount (overrides property default)';
COMMENT ON COLUMN client_property_assignments.show_nightly_rate IS 'Whether to display nightly rate on client page';
COMMENT ON COLUMN client_property_assignments.custom_nightly_rate IS 'Nightly rate amount';
COMMENT ON COLUMN client_property_assignments.show_purchase_price IS 'Whether to display purchase price on client page';
COMMENT ON COLUMN client_property_assignments.custom_purchase_price IS 'Purchase price amount';
`);
    } else if (error) {
      console.error('Error:', error.message);
    } else {
      console.log('✓ Pricing fields already exist!');
    }
  } catch (err) {
