...
export function PropertyContactForm({ propertyAddress, managers }: PropertyContactFormProps) {
  ...
  const defaultContact = {
    ...
    company: "Cadiz & Lluis",
    email: "contact@luxuryconcierge.com",
    phone: "(213) 555-0100"
  }

  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
      {/* Contact Info Card - Always show either manager or default */}
        ...
        <Card className="bg-card/50 border border-border/30 backdrop-blur-sm h-full">
          <CardHeader className="pb-4">
            <CardTitle className="luxury-heading text-xl text-white tracking-[0.15em]">
              {primaryManager ? 'Your Property Manager' : 'Contact Information'}
            </CardTitle>
          </CardHeader>
            ...
            <div className="text-center space-y-4">
              <div>
                <h3 className="text-2xl font-bold text-white tracking-wide">
                  {primaryManager ? primaryManager.name : defaultContact.name}
                </h3>
                {!primaryManager && (
                  <p className="text-white/70 text-sm mt-1 tracking-wider uppercase">
                    {defaultContact.company}
                  </p>
                )}
              </div>
              ...
              <div className="flex flex-col items-center gap-2 p-4 bg-white/5 rounded-lg border border-white/10">
                ...
                <a
                  href={`mailto:${primaryManager ? primaryManager.email : defaultContact.email}`}
                  className="text-white hover:text-white/80 transition-colors text-sm break-all"
                >
                  {primaryManager ? primaryManager.email : defaultContact.email}
                </a>
              </div>

              {/* Phone */}
              {(primaryManager?.phone || !primaryManager) && (
                <div className="flex flex-col items-center gap-2 p-4 bg-white/5 rounded-lg border border-white/10">
                  <div className="flex items-center gap-2 text-white/70">
                  ...
                  <a
                    href={`tel:${primaryManager ? primaryManager.phone : defaultContact.phone}`}
                    className="text-white hover:text-white/80 transition-colors text-sm"
                  >
                    {formatPhoneNumber(primaryManager ? primaryManager.phone! : defaultContact.phone)}
                  </a>
                </div>
              )}
            </div>
        ...
        <CardHeader className="pb-6">
          ...
          <CardDescription className="text-white/70 text-base">
            {primaryManager
              ...
              : 'Get in touch for more information about this property'
            }
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
